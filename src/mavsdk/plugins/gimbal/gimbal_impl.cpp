...
void GimbalImpl::request_gimbal_manager_information(uint8_t target_component_id)
{
    if (_debugging) {
...
void GimbalImpl::request_gimbal_device_information(uint8_t target_component_id)
{
    if (_debugging) {
...
void GimbalImpl::process_heartbeat(const mavlink_message_t& message)
{
    ...
    auto maybe_gimbal = std::find_if(_gimbals.begin(), _gimbals.end(), [&](GimbalItem& item) {
    ...
    auto* gimbal = [&]() {
        if (maybe_gimbal != _gimbals.end()) {
            ...
            for (auto& gimbal : _gimbals) {
                LogErr() << "gimbal: " << std::to_string(gimbal.gimbal_manager_compid) << ", "
                         << std::to_string(gimbal.gimbal_device_id);
            }
            LogErr() << "Now gimbals: DONE";
...
void GimbalImpl::check_is_gimbal_valid(GimbalItem* gimbal)
{
    ...
    // having tried multiple times, we might as well
    if (!gimbal->gimbal_device_information_received) {
...
void GimbalImpl::take_control_async(
    int32_t gimbal_id, Gimbal::ControlMode control_mode, Gimbal::ResultCallback callback)
{
    std::lock_guard<std::mutex> lock(_mutex);
...
void GimbalImpl::release_control_async(int32_t gimbal_id, Gimbal::ResultCallback callback)
{
    std::lock_guard<std::mutex> lock(_mutex);
...
{
    LogWarn() << "command_result: " << (int)command_result;
...
std::optional<GimbalImpl::GimbalAddress> GimbalImpl::maybe_address_for_gimbal_id(int32_t gimbal_id)
{
    // Assumes lock
